...
class TetrisGameModel implements GameModel {
    ...
    private List<Point> ghostPoints = new ArrayList<>();
    ...
    private void addGhostPoints() {
        ...
        for (Point fallingPoint : mFallingPoints) {
            ghostPoints.add(new Point(fallingPoint.x, fallingPoint.y, PointType.EMPTY, false));
        }
        ...
        while (canDrop) {
            ...
            canDrop = false;
            for (Point fallingPoint : mFallingPoints) {
                Point pointBelow = getPlayingPoint(fallingPoint.x, fallingPoint.y + dem);
                if (pointBelow == null || pointBelow.isStablePoint()) {
                    canDrop = false;
                    break;
                } else {
                    canDrop = true;
        ...
        List<Point> updatedGhostPoints = new ArrayList<>();
        for (Point fallingPoint : ghostPoints) {
            updatedGhostPoints.add(new Point(fallingPoint.x, fallingPoint.y + dem, PointType.GHOST, true));
        }
        ghostPoints.clear();
        ghostPoints.addAll(updatedGhostPoints);
        ghostPoints.forEach(this::updatePlayingPoint);
    }
        ...
        switch (turn) {
            case LEFT:
                ...
                if (canTurn) {
                    ...
                    mFallingPoints.addAll(tmPoints);
                    mFallingPoints.forEach(this::updatePlayingPoint);
                if(difficulty.equals("easy")){    addGhostPoints();}
                }
                break;
            case RIGHT:
                updateFallingPoints();
